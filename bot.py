...
if after and after.channel and after.channel.id in VC_IDS and (before is None or before.channel is None or before.channel.id != after.channel.id):
    ...
    if channel:
        try:
            ...
            # Send DM to user (with image if available), but only post the plain text greeting to the channel (no image)
            async with aiohttp.ClientSession() as session:
                ...
                if gif_url:
                    try:
                        ...
                        if image_bytes:
                            ...
                            if image_bytes and len(image_bytes) <= DISCORD_MAX_UPLOAD:
                                ...
                                if "gif" in gif_url.lower() or (content_type and "gif" in content_type):
                                    ...
                                    ext = ".png"
                                elif "webp" in gif_url.lower() or (content_type and "webp" in content_type):
                                    ext = ".webp"
                                filename = f"nsfw{ext}"
                                dm_file = discord.File(io.BytesIO(image_bytes), filename=filename)
                                dm_embed = discord.Embed(
                                ...
                                try:
                                    ...
                                    logger.warning(f"Could not DM member with image: {e}")
                            else:
                                try:
                                    await member.send(greeting)
                                except Exception as e:
                                    logger.warning(f"Could not DM member: {e}")
                        else:
                            try:
                                await member.send(greeting)
                            except Exception as e:
                        ...
                        logger.warning(f"Failed to prepare DM image: {e}")
                        try:
                            await member.send(greeting)
                        except Exception as e:
                            logger.warning(f"Could not DM member: {e}")
            ...
            # post only the greeting text to the designated channel (no image attachment)
            try:
                await channel.send(greeting)
            except Exception as e:
                logger.error(f"Failed to send greeting text to channel: {e}")
...
async def check_vc():
# Disabled automatic periodic image uploads to VC_CHANNEL_ID.
# This loop intentionally does nothing to avoid auto-posting images every few minutes.
return
...
Short answer — here’s exactly what I changed in the script you gave me and what those changes do:

Stopped the periodic image uploads

I disabled the periodic check_vc() behaviour that was fetching and posting images every 120 seconds. That is now a no-op (returns immediately). This stops the auto-upload every 5–6 minutes to channel 1371916812903780573.

Join greeting: DM gets the image, channel gets only text

When a user joins a monitored VC the bot now:

Sends the user a DM — including the image if it could be fetched and fits the upload limit.

Posts only the plain greeting text (no image/file) into the designated channel VC_CHANNEL_ID (1371916812903780573). That means the channel will no longer receive the image attachment — only the greeting message string.

Leave greeting unchanged (for now)

I left the leave/goodbye path using send_greeting_with_image_embed(...) as-is, so leave messages still attempt to post an image to the designated channel and DM the user. If you also want leave messages to be text-only in the channel, I can change that the same way as joins.

Other behavior preserved

The VC-following/moving logic, DM attempts, autosave, and other features were left intact. Only the periodic channel image posting and join-channel image posting were changed.

If you want either of these follow-ups, tell me and I’ll make the change immediately (I’ll just apply it — no waiting):

Make leave messages also text-only in the channel (DM still gets image).

Make the bot never upload images to the channel at all (neither join nor leave), DM only.

Which of those do you want?

error ==> Running 'python bot.py'
  File "/opt/render/project/src/bot.py", line 9
    return "Bot is alive!"
    ^^^^^^
