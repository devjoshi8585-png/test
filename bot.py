...
async def fetch_from_konachan(session, positive):
    try:
        ...
        if random.random() < 0.85:
            tags.append(random.choice(SPICY_TAGS).replace("_", " "))
        if random.random() < 0.60:
            tags.append("animated")
        base = "https://konachan.com/post.json"
        ...
        async with session.get(
            ...
            return gif_url, "konachan_spicy", post
    except Exception as e:
        if DEBUG_FETCH:
...
async def fetch_from_danbooru(session, positive):
    try:
        ...
        async with session.get(base, params=params, headers=headers or None, timeout=REQUEST_TIMEOUT) as resp:
            ...
            return gif_url, "danbooru_spicy", post
    except Exception:
        return None, None, None
    ...
    try:
        ...
        async with session.get(base, params=params, timeout=REQUEST_TIMEOUT) as resp:
            ...
            if not gif_url:
                return None, None, None
            extract_and_add_tags_from_meta(post.get("tags", ""), GIF_TAGS, data)
            return gif_url, "gelbooru_spicy", post
    except Exception:
        return None, None, None
...
async def fetch_from_e621(session, positive):
    try:
        ...
        async with session.get(base, params=params, headers=headers, auth=auth, timeout=REQUEST_TIMEOUT) as resp:
            ...
            extract_and_add_tags_from_meta(tag_string, GIF_TAGS, data)
            return gif_url, "e621_spicy", post
    except Exception as e:
        if DEBUG_FETCH:
    ...
    try:
        ...
        async with session.get(
            ...
            if not gif_url:
                return None, None, None
            extract_and_add_tags_from_meta(post.get("tags", ""), GIF_TAGS, data)
            return gif_url, "yandere_spicy", post
    except Exception as e:
        if DEBUG_FETCH:
